...
export function useAuth() {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      if (firebaseUser) {
        ...
        try {
          ...
          if (!docSnap.exists()) {
            ...
            const newUser: VyntraUser = {
              ...
              avatar: firebaseUser.photoURL || `https://ui-avatars.com/api/?name=${firebaseUser.displayName}&background=6C5CE7&color=fff`,
              bio: "New to Vyntra",
              followersCount: 0,
              followingCount: 0,
              verified: false,
        ...
        unsubscribeUserDoc = onSnapshot(userRef, (docSnap) => {
          if (docSnap.exists()) {
            setUser(docSnap.data() as VyntraUser);
          } else {
             // Fallback
             setUser({
                 uid: firebaseUser.uid,
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/popup-blocked' || error.code === 'auth/cancelled-popup-request') {
         alert("Sign in popup blocked or cancelled. For Google login, please open the app in a new tab (using the button in the top right header) or use Email login instead.");
      } else {
  ...
  const loginWithApple = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/popup-blocked' || error.code === 'auth/cancelled-popup-request') {
         alert("Apple sign in popup blocked or cancelled. Please open the app in a new tab (using the button in the top right header) or use Email login instead.");
      } else {
         alert(`Apple sign in error: ${error.message}. Make sure Apple Sign-In is configured in Firebase Auth.`);
      }
    }
  };
  ...
  return { user, loading, loginWithGoogle, loginWithApple, logout, loginWithEmail, signupWithEmail, updateUserBio, updateUserProfile, updateUserPreferences };
}
