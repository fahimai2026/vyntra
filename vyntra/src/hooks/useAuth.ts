      ...
      if (firebaseUser) {
        ...
        if (!docSnap.exists()) {
          const newUser: VyntraUser = {
            ...
            name: firebaseUser.displayName || 'Vyntra User',
            handle: '@' + (firebaseUser.email?.split('@')[0] || firebaseUser.uid.substring(0, 8)),
            avatar: firebaseUser.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(firebaseUser.displayName || 'Vyntra User')}&background=6C5CE7&color=fff`,
            bio: "New to Vyntra",
            followersCount: 0,
            followingCount: 0,
            verified: false,
          ...
          // Sync with local SQL database quietly
          await fetch('/api/users/sync', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(newUser)
          }).catch(syncErr => console.warn("Failed silent Postgres sync on popup register:", syncErr));
        }
      }
      return firebaseUser;
    } catch (error: any) {
