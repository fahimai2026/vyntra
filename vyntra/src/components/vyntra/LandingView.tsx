...
export function LandingView() {
  const { loginWithEmail, signupWithEmail, signInWithGoogle, signInWithGoogleRedirect } = useAuth();
  ...
  const handleGoogleSignInWithPopup = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/unauthorized-domain') {
        ...
        triggerToast("Redirecting to Google Sign-In...", "info");
        try {
          await signInWithGoogleRedirect();
        } catch (redirectError: any) {
          console.error("Redirect fallback failed:", redirectError);
          triggerToast("Failed to redirect to Google.", "error");
          setGoogleIsLoading(false);
        }
      } else {
