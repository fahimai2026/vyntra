...
export function LandingView() {
  ...
  const handleGoogleSignInWithPopup = async () => {
    ...
    try {
      ...
      triggerToast("Authenticated successfully with Google popup!", "success");
    } catch (error: any) {
      console.error("Google Popup Auth failed:", error);
      if (error.code === 'auth/unauthorized-domain') {
        setUnauthorizedDomain(window.location.hostname);
      } else if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/popup-blocked' || error.code === 'auth/cancelled-popup-request') {
        triggerToast("Google sign-in popup blocked or closed. Please try again or open the application directly in a new browser tab.", "info");
      } else {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!resolveRes.ok) {
        ...
        throw new Error(errData.error || "No account found with this username or email.");
      }
  ...
  return (
    <div className={`flex flex-col lg:flex-row min-h-screen w-full ${
      ...
      <div className="fixed top-4 right-4 left-4 sm:left-auto sm:top-6 sm:right-6 sm:max-w-sm z-50 flex flex-col gap-3 pointer-events-none">
        <AnimatePresence>
          {toasts.map((toast) => (
            <motion.div
              key={toast.id}
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5 w-full">
                  {authStep !== 'otp' && (
                    <div className={`flex flex-col gap-2.5 w-full border-b pb-4 mb-2 ${
                      ...
                      >
                        <GoogleLogo className="w-4.5 h-4.5 flex-shrink-0" />
                        <span className="text-xs">{isLogin ? 'Sign in with Google' : 'Sign up with Google'}</span>
                      </button>
