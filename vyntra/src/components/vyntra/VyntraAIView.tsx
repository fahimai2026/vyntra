...
export function VyntraAIView() {
  ...
  const syncTwin = async () => {
    ...
    try {
      // 1. Fetch user posts from client-side Firestore safely
      let postsList: any[] = [];
      try {
        const { collection, getDocs, query, where } = await import('firebase/firestore');
        const { db } = await import('../../firebase');
        const postsSnap = await getDocs(query(collection(db, 'posts'), where('authorId', '==', user.uid)));
        postsList = postsSnap.docs.map(doc => ({
          content: doc.data().content || "",
          mediaUrl: doc.data().mediaUrl || null
        }));
      } catch (fErr) {
        console.warn("Could not retrieve Firestore user posts for sync payload:", fErr);
      }

      // 2. Post payload to backend sync endpoint
      await fetch(`/api/users/${user.uid}/twin/sync`, { 
        method: 'POST',
